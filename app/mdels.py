...
class Departments(db.Model):
    ...
    lectures = db.relationship('Lectures', backref=db.backref('department_id', lazy=True))
...
class Students(db.Model):
    ...
    enrolled_student = db.relationship('Enrollments', backref=db.backref('student_id'), lazy=True)
...
class Units(db.Model):
    ...
    status=db.Column(db.String(20), nullable=False)
    Y_id=db.Column(db.Integer, db.ForeignKey('Years.id',nullable=False))
    Lecturer_id=db.Column(db.ForeignKey('Lecturers.id'),nullable=False)
